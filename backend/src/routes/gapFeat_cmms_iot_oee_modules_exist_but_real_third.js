...
// Gap Feature: CMMS, IoT, OEE modules exist but real third
// CMMS, IoT, OEE modules exist but real third-party adapters look like scaffolding
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / gap-cmms-iot-oee-modules-exist-but-real-third',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'CMMS, IoT, OEE modules exist but real third'.\nProject: AIPredictiveMaintenance.\nContext: CMMS, IoT, OEE modules exist but real third-party adapters look like scaffolding\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['cmms-iot-oee-modules-exist-but-real-third', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'cmms-iot-oee-modules-exist-but-real-third', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['cmms-iot-oee-modules-exist-but-real-third']
    );
