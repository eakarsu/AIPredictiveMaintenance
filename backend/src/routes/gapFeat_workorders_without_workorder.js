...
// Gap Feature: Workorders without \'/workorder
// Workorders without /workorder-priority-optimize (rank work by impact)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / gap-workorders-without-workorder',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Workorders without \'/workorder'.\nProject: AIPredictiveMaintenance.\nContext: Workorders without /workorder-priority-optimize (rank work by impact)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['workorders-without-workorder', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'workorders-without-workorder', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
