...
// AI model of equipment behavior; simulate different maintenance strategies (PM interval, corrective timing); predict long-term costs and downtime
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / cf-digital-twin-simulation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_digital_twin_simulation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-digital-twin-simulation feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Digital twin simulation'.
Context: AI model of equipment behavior; simulate different maintenance strategies (PM interval, corrective timing); predict long-term costs and downtime
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-digital-twin-simulation/run error:', err);
