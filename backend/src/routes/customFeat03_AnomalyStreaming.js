...
// Continuous monitoring with real-time alerting; anomaly is surfaced immediately with root cause hypothesis (bearing wear, corrosion, misalignment)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / cf-anomaly-streaming',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_anomaly_streaming`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-anomaly-streaming feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Anomaly streaming'.
Context: Continuous monitoring with real-time alerting; anomaly is surfaced immediately with root cause hypothesis (bearing wear, corrosion, misalignment)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-anomaly-streaming/run error:', err);
