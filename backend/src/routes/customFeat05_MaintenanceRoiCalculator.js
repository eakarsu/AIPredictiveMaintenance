...
// Compare maintenance strategy costs (PM, corrective, CM) vs. production losses; recommend optimal strategy per asset
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / cf-maintenance-roi-calculator',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_maintenance_roi_calculator`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-maintenance-roi-calculator feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Maintenance ROI calculator'.
Context: Compare maintenance strategy costs (PM, corrective, CM) vs. production losses; recommend optimal strategy per asset
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-maintenance-roi-calculator/run error:', err);
