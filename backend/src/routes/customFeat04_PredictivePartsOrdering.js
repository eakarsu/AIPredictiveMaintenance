...
// AI predicts parts failure; auto-orders replacements to arrive before failure; prevents emergency expedites and downtime
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / cf-predictive-parts-ordering',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_predictive_parts_ordering`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-predictive-parts-ordering feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Predictive parts ordering'.
Context: AI predicts parts failure; auto-orders replacements to arrive before failure; prevents emergency expedites and downtime
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-predictive-parts-ordering/run error:', err);
