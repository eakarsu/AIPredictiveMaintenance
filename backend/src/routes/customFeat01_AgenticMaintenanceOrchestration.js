...
// Real-time monitoring of all sensors; AI predicts failures with confidence scoring; auto-schedules maintenance before failure; optimizes technician routing and parts availability
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / cf-agentic-maintenance-orchestration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_maintenance_orchestration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-maintenance-orchestration feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic maintenance orchestration'.
Context: Real-time monitoring of all sensors; AI predicts failures with confidence scoring; auto-schedules maintenance before failure; optimizes technician routing and parts availability
Project: AIPredictiveMaintenance.
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-maintenance-orchestration/run error:', err);
