...
// Gap Feature: Alerts without \'/alert
// Alerts without /alert-fatigue-reduce (suppress low-priority alerts)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPredictiveMaintenance / gap-alerts-without-alert',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Alerts without \'/alert'.\nProject: AIPredictiveMaintenance.\nContext: Alerts without /alert-fatigue-reduce (suppress low-priority alerts)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['alerts-without-alert', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'alerts-without-alert', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
