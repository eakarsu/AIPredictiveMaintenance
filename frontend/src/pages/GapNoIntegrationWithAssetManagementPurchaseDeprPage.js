...
// Gap (Non-AI): No integration with asset management (purchase, depreciation)
// No integration with asset management (purchase, depreciation)
import React, { useState } from 'react';
import { apiFetch } from '../services/apiFetch';
import AIResultDisplay from '../components/AIResultDisplay';

export default function GapNoIntegrationWithAssetManagementPurchaseDeprPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/gap-no-integration-with-asset-management-purchase-depr/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No integration with asset management (purchase, depreciation)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No integration with asset management (purchase, depreciation)</p>
