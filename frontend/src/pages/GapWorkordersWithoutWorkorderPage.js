...
// Gap (AI): Workorders without '/workorder
// Workorders without /workorder-priority-optimize (rank work by impact)
import React, { useState } from 'react';
import { apiFetch } from '../services/apiFetch';
import AIResultDisplay from '../components/AIResultDisplay';

export default function GapWorkordersWithoutWorkorderPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/gap-workorders-without-workorder/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Workorders without '/workorder</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Workorders without /workorder-priority-optimize (rank work by impact)</p>
