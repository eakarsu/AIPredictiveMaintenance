...
// Gap (Non-AI): Limited notifications layer
// Limited notifications layer
import React, { useState } from 'react';
import { apiFetch } from '../services/apiFetch';
import AIResultDisplay from '../components/AIResultDisplay';

export default function GapLimitedNotificationsLayerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/gap-limited-notifications-layer/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited notifications layer</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited notifications layer</p>
