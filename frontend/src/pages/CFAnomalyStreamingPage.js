...
// Custom Feature: Anomaly streaming
// Anomaly streaming: Continuous monitoring with real-time alerting; anomaly is surfaced immediately with root cause hypothesis (bearing wear, corrosion, misalignment)
import React, { useState } from 'react';
import { apiFetch } from '../services/apiFetch';
import AIResultDisplay from '../components/AIResultDisplay';

export default function CFAnomalyStreamingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/cf-anomaly-streaming/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Anomaly streaming</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Anomaly streaming: Continuous monitoring with real-time alerting; anomaly is surfaced immediately with root cause hypothesis (bearing wear, corrosion, misalignment)</p>
